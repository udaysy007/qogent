...
INSERT INTO blog_tags (name) VALUES ('Banking') ON CONFLICT (name) DO NOTHING;
...
INSERT INTO blog_tags (name) VALUES ('Money Management') ON CONFLICT (name) DO NOTHING;
...
DECLARE
  post_id UUID;
BEGIN
  SELECT id INTO post_id FROM blog_posts WHERE slug = 'international-student-banking-guide';
  ...
  IF post_id IS NOT NULL THEN
    ...
    INSERT INTO blog_posts_tags (post_id, tag_id)
    SELECT post_id, id FROM blog_tags WHERE name = 'Banking';
    ...
    INSERT INTO blog_posts_tags (post_id, tag_id)
    SELECT post_id, id FROM blog_tags WHERE name = 'Money Management';
  END IF;
END;
$$; 