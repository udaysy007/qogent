...
-- Adding 2 more Irish universities
INSERT INTO universities (name, slug, country_id, city, is_public, ranking, description, website, logo_url)
VALUES 
  ('University College Dublin', 'university-college-dublin', 
   (SELECT id FROM countries WHERE slug = 'ireland'), 
   'Dublin', true, 173, 
   'University College Dublin is Ireland''s largest university with a diverse international community. Founded in 1854, it has played a key role in Ireland''s educational and cultural development and is known for its strong research programs.',
   'https://www.ucd.ie/', '/images/universities/ucd-logo.svg'),
...
-- Add campus images for the Irish universities
UPDATE universities 
...
VALUES 
  ('Computer Science', 
   (SELECT id FROM universities WHERE slug = 'university-college-dublin'), 
   'master', 
   ...
   '1 year',
   'English',
   '{"amount": 25600, "currency": "EUR", "period": "year", "notes": "Non-EU student rate"}'),
  ...
  ('Business Analytics', 
   ...
   '1 year',
   'English',
   '{"amount": 24800, "currency": "EUR", "period": "year", "notes": "Non-EU student rate"}');

-- Add programs for NUI Galway
INSERT INTO programs (name, university_id, degree_level, title, description, duration, language, tuition_fee)
VALUES 
  ('Data Analytics', 
   (SELECT id FROM universities WHERE slug = 'national-university-ireland-galway'), 
   'master', 
   ...
   '1 year',
   'English',
   '{"amount": 19850, "currency": "EUR", "period": "year", "notes": "Non-EU student rate"}'),
  ...
  ('Biomedical Engineering', 
   ...
   '1 year',
   'English',
   '{"amount": 22350, "currency": "EUR", "period": "year", "notes": "Non-EU student rate"}'); 